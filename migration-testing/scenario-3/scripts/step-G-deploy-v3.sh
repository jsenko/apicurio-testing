...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    if docker inspect scenario3-postgres-v3 --format='{{.State.Health.Status}}' 2>/dev/null | grep -q "healthy"; then
        echo "" | tee -a "$LOG_FILE"
...
if [ $ELAPSED -ge $MAX_WAIT ]; then
    ...
    docker logs scenario3-postgres-v3 2>&1 | tee -a "$LOG_FILE"
    exit 1
fi
...
echo "  docker logs scenario3-postgres-v3" | tee -a "$LOG_FILE"
echo "  docker logs scenario3-registry-v3" | tee -a "$LOG_FILE"

# Collect initial container logs
echo "" | tee -a "$LOG_FILE"
echo "Collecting initial container logs..." | tee -a "$LOG_FILE"
docker logs scenario3-postgres-v3 > "$PROJECT_DIR/logs/containers/postgres-v3-initial.log" 2>&1 || true
docker logs scenario3-registry-v3 > "$PROJECT_DIR/logs/containers/registry-v3-initial.log" 2>&1 || true
