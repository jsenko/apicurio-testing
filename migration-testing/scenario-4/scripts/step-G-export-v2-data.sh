...
echo "[1/5] Authenticating with Keycloak..." | tee -a "$LOG_FILE"
...
echo "[2/5] Checking registry accessibility..." | tee -a "$LOG_FILE"
if ! curl -f -s -k -H "Authorization: Bearer $ACCESS_TOKEN" "$REGISTRY_URL/apis/registry/v2/system/info" > /dev/null 2>&1; then
...
if [ -f "$EXPORT_FILE" ]; then
    echo "[3/5] Removing old export file..." | tee -a "$LOG_FILE"
    rm -f "$EXPORT_FILE"
    echo "  ✓ Old export file removed" | tee -a "$LOG_FILE"
else
    echo "[3/5] No old export file to remove" | tee -a "$LOG_FILE"
fi
echo "" | tee -a "$LOG_FILE"

# Export data from registry
echo "[4/5] Exporting data from registry v2..." | tee -a "$LOG_FILE"
echo "  Calling: $EXPORT_ENDPOINT" | tee -a "$LOG_FILE"

# Use curl to download the export zip file with authentication
HTTP_CODE=$(curl -w "%{http_code}" -o "$EXPORT_FILE" -s -k -H "Authorization: Bearer $ACCESS_TOKEN" "$EXPORT_ENDPOINT")
...
# Verify export file
echo "[5/5] Verifying export file..." | tee -a "$LOG_FILE"
