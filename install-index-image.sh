...
TARGET_REGISTRY_HOST=$(kubectl get route -n infra image-registry --template='{{ .spec.host }}')
PASSWORD=$(cat "$CLUSTER_DIR/image-registry-admin-password")

if [[ -z "$TARGET_REGISTRY_HOST" || -z "$PASSWORD" ]]; then
  error_exit "Image registry is not properly configured in the target cluster."
else
  success "Image registry is available at $TARGET_REGISTRY_HOST ."
fi

if ! docker login -u admin -p "$PASSWORD" "$TARGET_REGISTRY_HOST"; then
  error_exit "Docker login to image registry at $TARGET_REGISTRY_HOST failed."
fi
...
TARGET_BUNDLE_IMAGE="$TARGET_REGISTRY_HOST/$SOURCE_BUNDLE_IMAGE_PATH:sha256-$SOURCE_BUNDLE_IMAGE_SHA_HASH"
...
if ! docker push "$TARGET_BUNDLE_IMAGE"; then
  error_exit "Failed to push image $TARGET_BUNDLE_IMAGE to the registry at $TARGET_REGISTRY_HOST ."
else
...
TARGET_IMAGE="$TARGET_REGISTRY_HOST/$SOURCE_IMAGE_PATH:$SOURCE_IMAGE_TAG"

export PLACEHOLDER_SOURCE_IMAGE="$SOURCE_IMAGE"
export PLACEHOLDER_TARGET_HOST="$TARGET_REGISTRY_HOST"
...
if ! docker push "$TARGET_IMAGE"; then
  error_exit "Failed to push image $TARGET_IMAGE to the registry at $TARGET_REGISTRY_HOST ."
else
