...
# Container logs
docker logs scenario4-postgres-v2
docker logs scenario4-registry-v2
docker logs scenario4-nginx
...
```bash
# List all scenario4 containers
docker ps -a | grep scenario4

# Check health
docker ps --format "table {{.Names}}\t{{.Status}}"

# Detailed inspect
docker inspect scenario4-registry-v2 | jq '.[0].State'
```
...
```bash
# List networks
docker network ls | grep scenario4

# Inspect network
docker network inspect scenario4-v2-network

# Recreate network
docker network rm scenario4-v2-network
docker network create scenario4-v2-network
```
