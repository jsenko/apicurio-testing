...
# Step R: Run Kafka Producer v3
#
# This script:
# 1. Runs the kafka-producer-v3 application against Registry v3
# 2. Produces messages to the 'avro-messages' topic
# 3. Tests v3 SerDes with Registry v3 (with TLS and Auth)
...
LOG_FILE="$LOG_DIR/step-R-run-producer-v3.log"
...
REGISTRY_URL="https://localhost:8443/apis/registry/v3"
...
if ! docker exec scenario4-kafka /opt/kafka/bin/kafka-broker-api-versions.sh --bootstrap-server localhost:9092 > /dev/null 2>&1; then
    log "  Kafka is not running. Please run step-R-deploy-kafka.sh first."
    exit 1
fi
...
SCHEMAS=$(curl -s -k https://localhost:8443/apis/registry/v3/search/artifacts?group=default | jq -r '.artifacts[].artifactId' 2>/dev/null || echo "")
if [ -n "$SCHEMAS" ]; then
...
log "  curl -k https://localhost:8443/apis/registry/v3/search/artifacts?group=default | jq"
log ""
