...
RETRY_COUNT=0
MAX_RETRIES=30
until docker exec scenario3-postgres-v2 pg_isready -U apicurio -d registry > /dev/null 2>&1; do
...
echo "  docker logs scenario3-postgres-v2" | tee -a "$LOG_FILE"
echo "  docker logs scenario3-registry-v2" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"

# Collect initial container logs
echo "Collecting initial container logs..." | tee -a "$LOG_FILE"
docker logs scenario3-postgres-v2 > "$PROJECT_DIR/logs/containers/postgres-v2-initial.log" 2>&1
docker logs scenario3-registry-v2 > "$PROJECT_DIR/logs/containers/registry-v2-initial.log" 2>&1
