...
# Step S: Run Kafka Consumer v3
#
# This script:
# 1. Runs the kafka-consumer-v3 application
# 2. Consumes messages from the 'avro-messages' topic
# 3. Verifies messages can be deserialized using the schema from Registry v3 (with TLS and Auth)
...
LOG_FILE="$LOG_DIR/step-S-run-consumer-v3.log"
REPORT_FILE="$DATA_DIR/consumer-v3-report.txt"
...
# Run consumer
log "[3/3] Running Kafka Consumer v3..."
...
timeout ${TIMEOUT_SECONDS}s java -jar "$JAR_PATH" 2>&1 | tee -a "$LOG_FILE" > "$REPORT_FILE"
