...
if ! docker ps | grep -q scenario4-registry-v2; then
...
echo "  docker logs scenario4-nginx" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"

# Collect nginx logs
echo "Collecting nginx logs..." | tee -a "$LOG_FILE"
docker logs scenario4-nginx > "$PROJECT_DIR/logs/containers/nginx-initial.log" 2>&1
